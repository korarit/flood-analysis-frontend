  ...
  try {
    ...
    if (baseStation && relations) {
      const mappedInfluencing: StationRelation[] = (relations.influencingRainfallStations || []).map((inf: any) => ({
        ...
        travelTimeMinutes: inf.travelTimeMinutes ?? (inf.travelTimeHours ? Math.round(inf.travelTimeHours * 60) : undefined),
        travelTimeMinutesMin: inf.travelTimeMinutesMin,
        travelTimeMinutesMax: inf.travelTimeMinutesMax,
        travelTimeHoursMin: inf.travelTimeHoursMin,
        travelTimeHoursMax: inf.travelTimeHoursMax,
        influenceWeightPercent: inf.influenceWeightPercent,
      ...
      const mappedDownstream: StationRelation[] = (relations.streamFall || relations.downstreamStations || []).map((ds: any) => ({
        ...
        travelTimeMinutes: ds.travelTimeMinutes ?? (ds.travelTimeHours ? Math.round(ds.travelTimeHours * 60) : undefined),
        travelTimeMinutesMin: ds.travelTimeMinutesMin,
        travelTimeMinutesMax: ds.travelTimeMinutesMax,
        travelTimeHoursMin: ds.travelTimeHoursMin,
        travelTimeHoursMax: ds.travelTimeHoursMax,
        influenceWeightPercent: ds.influenceWeightPercent,
      ...
      if (relations.relations && relations.relations.length > 0) {
        const generalRelations: StationRelation[] = relations.relations.map((r: any) => ({
          ...
          travelTimeMinutes: r.travelTimeMinutes ?? (r.travelTimeHours ? Math.round(r.travelTimeHours * 60) : undefined),
          travelTimeMinutesMin: r.travelTimeMinutesMin ?? undefined,
          travelTimeMinutesMax: r.travelTimeMinutesMax ?? undefined,
          travelTimeHoursMin: r.travelTimeHoursMin ?? undefined,
          travelTimeHoursMax: r.travelTimeHoursMax ?? undefined,
          influenceWeightPercent: r.influenceWeightPercent ?? undefined,
